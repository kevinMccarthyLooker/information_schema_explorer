...
view: information_schema__columns_matching {
  derived_table: {
    sql:
    ...
    /*some  liquid here could help with speed, but will be inflexible */
    /*
    where
    --matching_columns.table_schema in ('looker_scratch','public')
    ...
    */
    ...
    where
    --columns.table_schema in ('looker_scratch','public')
    --and
    {% condition information_schema__columns.table_schema %}columns.table_schema{% endcondition %}
    and
    {% condition information_schema__columns.table_name %}columns.table_name{% endcondition %}
    and
    {% condition information_schema__columns.column_name %}columns.column_name{% endcondition %}
    )
    --limit to only fields of interest...
    where
    match_type = 'self'
    or
...
#     https://vydia.looker.com/explore/z_information_schema_columns/profiling__compare_two_fields?toggle=fil&qid=l5b5SwxMWWKpNXNrwDLVxv
    ...
    dimension: table_name__column_name {hidden:yes label:" Table__Column"  }
...
#
    dimension: table_catalog {hidden:yes group_label:" Schema Info" }
    dimension: table_schema {hidden:yes group_label:" Schema Info" }
